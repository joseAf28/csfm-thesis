...
# import tensorflow_gan as tfgan ###! bypass the import for now
import logging
...
from target_flow_lib import OTFlow
...
def train(config, workdir):
    ...
    ###* logic here hardcodes specific architectural constrainsts for certain datasets.
    ...
    ####! --- CSFM MODIFICATION START ---
    mode = getattr(config.training, 'mode', 'sde')
    # 1. Initialize Physics Engine (Target Flow)
    # We only need this for CSFM mode
    if mode == 'csfm':
        flow_engine = TargetCompositeFlow(
            img_shape=(config.data.num_channels, config.data.image_size, config.data.image_size),
            device=config.device,
            modes=config.training.flow_modes,
    ...
    ##! NEW CODE TO BE USED
    # 1. Initialize Optimization Manager (Shared)
    # This ensures both modes use the same Gradient Clipping & Warmup logic
    optimize_fn = losses.optimization_manager(config)
    reduce_mean = config.training.reduce_mean
    
    # 3. Conditional Switching (The Logic)
    if mode == 'csfm' or mode == 'ot':
        # --- CSFM Mode ---
        # We pass flow_engine and regularizer instead of sde
        train_step_fn = losses.get_flow_step_fn(
        ...
        eval_step_fn = losses.get_flow_step_fn(
            flow_engine=flow_engine,
            regularizer=regularizer,
            train=False,
            optimize_fn=optimize_fn,
...
##* Not used so far: focus on representation learning 
def evaluate(config,
             workdir,
             eval_folder="eval"):
    ...
    # --- CSFM MODIFICATION START ---
    if getattr(config.training, 'mode', 'sde') == 'csfm':
        flow_engine = TargetCompositeFlow(
            img_shape=(config.data.num_channels, config.data.image_size, config.data.image_size),
            device=config.device
        )
    ...
    regularizer = LatentRegularizer(
        feature_dim=getattr(config.model, 'latents', 512),
        mode=getattr(config.training, 'reg_mode', 'sigreg'),
    ).to(config.device)
    # --- CSFM MODIFICATION END ---
    ...
    if config.eval.enable_loss:
        ...
        if getattr(config.training, 'mode', 'sde') == 'csfm':
            ...
            eval_step = losses.get_flow_step_fn(
                flow_engine=flow_engine,
                regularizer=regularizer,
                train=False,
                optimize_fn=optimize_fn,
