...
### the affiene transformation is not learnt and corresponds to the embedding vectors
...
class InstanceNorm2dPlus(nn.Module):
  ...
  def forward(self, x):
    ## x: (n,c,h,w)
    means = torch.mean(x, dim=(2, 3)) ## (n,c)
    m = torch.mean(means, dim=-1, keepdim=True) ## (n,1)
    v = torch.var(means, dim=-1, keepdim=True) ## (n, 1)
    means = (means - m) / (torch.sqrt(v + 1e-5)) ## (n,c)
    h = self.instance_norm(x) ## (n, c, h, w)
