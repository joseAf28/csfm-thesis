...
### modify the standard initialization
### init used for architectures with residual connections and makes the new block behave close to an 
### identity function at the beginning of the training
...
#### variance scaling - keep the variance of activations and gradients roughly constant as they propagate through the net layers
#### example: Xavier and Kaiming
...
### fundamental unit for feature extraction. It uses residual connections to allow for the stable training
### nstages of Activation -> 3x3 Convolution
### shortcut connection id added after each block - core principal of residual networks
...
### differebce for the before case: apply normalization: tajes the feature map x and the conditioning vector y as input
### normalization statisitcs to be influenced by the condition y
...
#### multi-scale fusion
#### feature maps from different resolutions - different layers of the encoder
#### fuse this multi-scale features into a single, unified feature map
class MSFBlock(nn.Module):
    def __init__(self, in_planes, features):
        ...
        ### create a 3x3 convolution for each input stream
        for i in range(len(in_planes)):
            self.convs.append(ncsn_conv3x3(in_planes[i], features, stride=1, bias=True))

    def forward(self, xs, shape):
        ### xs is a list of input tensors
        ### shape is the target (height, width)
...
#### composite blocks 
#### it takes multiple feature maps (likely from different levels of an encoder), processes them, fuses them
#### and produces a single refined output feature map.
#### rcublock - initially process one of the input feature maps
#### msfblock - fuse the outputs from the adapt_convs
#### crpblock - capture rich context from the fused feature map

class RefineBlock(nn.Module):
    def __init__(self, in_planes, features, act=nn.ReLU(), start=False, end=False, maxpool=True):
        super().__init__()
        ### in_planes :list of channel dimensions for each input it will receive
...
### resampling and residual blocks
#### convolve then pool
...
### norm --> act --> comv ; twice with a shortcut connction from the input to the output
...
def get_timestep_embedding(timesteps, embedding_dim, max_positions=10000):
    ...
    if embedding_dim % 2 == 1:  # zero pad in case of odd number 
...
### summing the last axis of x with the first axis of y : A_ij B_jk -> (AB)_ik
...
### net in net - micro network 
### 
class NIN(nn.Module):
    def __init__(self, in_dim, num_units, init_scale=0.1):
        super().__init__()
        self.W = nn.Parameter(default_init(scale=init_scale)((in_dim, num_units)), requires_grad=True) # (c, c')
        self.b = nn.Parameter(torch.zeros(num_units), requires_grad=True)

    def forward(self, x):
        x = x.permute(0, 2, 3, 1) # (b, c, h, w) to (b, h, w, c)
        y = contract_inner(x, self.W) + self.b # for each batch pixel: (b,h,w) we are multiplying by c to c'
...
class AttnBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        k = self.NIN_1(h)
        v = self.NIN_2(h)
        ### h, q, k, v all with (b,c,h,w)

        w = torch.einsum('bchw,bcij->bhwij', q, k) * (int(C) ** (-0.5)) ## similarity across the channel-dimension
